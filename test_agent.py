...
class DQNAgent:
    ...
    def __init__(self, states, actions, max_memory, double_q):
        ...
        # best v2 performance on model 4500000
        # new_saver = tf.train.import_meta_graph('models_DQN_v2/model-4500000.meta')
        # new_saver.restore(self.session, tf.train.latest_checkpoint('./models_DQN_v2'))

        # v3
        new_saver = tf.train.import_meta_graph('models_DQN_v3/model-8500000.meta')
        new_saver.restore(self.session, tf.train.latest_checkpoint('./models_DQN_v3'))

        # v4
        # new_saver = tf.train.import_meta_graph('models_DQN_v4/model-3500000.meta')
        # new_saver.restore(self.session, tf.train.latest_checkpoint('./models_DQN_v4'))
        ...
        # placeholders = [ op for op in graph.get_operations() if op.type == "BiasAdd"]
        # print(placeholders)
